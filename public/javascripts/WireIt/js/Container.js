...
(function() {
    ...
    WireIt.Container.prototype = {
        ...
        render: function() {
            ...
            //Create the element
            this.el = WireIt.cn('div', {className: 'portlet box grey-gallery ' + this.className});
            ...
            if(this.ddHandle) {
                var iconClass = "fa-cog";

                if (this.storm_type == "spout") iconClass = "fa-arrow-circle-down";
                if (this.storm_type == "bolt") iconClass = "fa-bolt";

                // Create the drag/drop handle
                this.ddHandle = WireIt.cn('div', {className: 'portlet-title ' + this.ddHandleClassName}, null,
                    '<div class="caption"> \
                        <i class="fa ' + iconClass + '"></i> ' + this.title +' \
                    </div> \
                    <div class="tools"> \
                    </div>'
                );
            ...
            this.bodyEl = WireIt.cn('div', {className: "portlet-body form"}, null, '<form class="" role="form"><div class="form-body"></div></form>');
            this.el.appendChild(this.bodyEl);
            this.bodyEl = $(this.bodyEl).find('.form-body')[0];
            ...
            var tools = $(this.el).find('.tools')[0];

            //this.collapseButton = WireIt.cn('a', {href : "",  className: "collapse"});
            //tools.appendChild(this.collapseButton);
            //Event.addListener(this.collapseButton, "click", this.onCollapse, this, true);

            if(this.close) {
                // Close button
                this.closeButton = WireIt.cn('a', {href : "",  className: "remove" + ' ' + this.closeButtonClassName});
                tools.appendChild(this.closeButton);
                Event.addListener(this.closeButton, "click", this.onCloseButton, this, true);
            }

            //if(this.groupable && this.ddHandle) {
            //    this.groupButton = WireIt.cn('div', {className: 'WireIt-Container-groupbutton'}, null, "groupable" );
