...
(function() {
   ...
   var lang = YAHOO.lang, CSS_PREFIX = "WireIt-";
    ...
    WireIt.TerminalProxy = function(terminal, options) {
        ...
        this.emitedFields = terminal.container.emitedFields || [];
        this.requiredFields = terminal.container.requiredFields || [];
    ...
    lang.extend(WireIt.TerminalProxy, YAHOO.util.DDProxy, {
        ...
        isValidWireTerminal: function(DDterminal) {
            ...
            // Check the allowSelfTopology
            if(this.terminal.container) {
                if(this.terminal.container.preventSelfTopology) {
            ...
            return this.checkAllRequiredFields(DDterminal);
        },

        checkAllRequiredFields: function(DDterminal) {

            var requiredFields, emitedFields;

            if (this.termConfig.type === "input") {
                requiredFields = this.requiredFields;
                emitedFields = DDterminal.emitedFields;
            } else if (this.termConfig.type === "output") {
                requiredFields = DDterminal.requiredFields;
                emitedFields = this.emitedFields;
            } else {
                return false;
            }

            if (requiredFields.length == 0) {
                console.log("requiredFields size: 0")
                return false;
            }

            for(var i = 0 ; i < requiredFields.length ; i++) {
                if ($.inArray(requiredFields[i], emitedFields) < 0) {
                    console.log("Required field not found ["+requiredFields[i]+"]")
                    return false;
                }
            }
