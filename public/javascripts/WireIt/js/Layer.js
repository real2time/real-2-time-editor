...
WireIt.Layer.prototype = {
    ...
    * @method setTopology
    * @param {Object} topology layer configuration
    */
   setTopology: function(topology) {
      this.clear();
      var i;
      if(YAHOO.lang.isArray(topology.containers)) {
         for(i = 0 ; i < topology.containers.length ; i++) {
            this.addContainer(topology.containers[i]);
         }
      }
      if(YAHOO.lang.isArray(topology.wires)) {
         for(i = 0 ; i < topology.wires.length ; i++) {
            this.addWire(topology.wires[i]);
         }
       }
   },
